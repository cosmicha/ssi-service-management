...
<div class="max-w-7xl mx-auto p-6">
    <div class="flex justify-between items-start mb-6">
        <div>
            <h1 class="text-3xl font-black">Incident Report</h1>
            <p class="text-slate-500">Corrective maintenance ticket report.</p>
        </div>
        <a href="{{ route('reports.export','incidents') }}?{{ http_build_query(request()->all()) }}" class="px-5 py-3 rounded-2xl bg-slate-900 text-white font-black">Export CSV</a>
    </div>
    ...
    <div class="bg-white rounded-3xl border shadow-sm overflow-x-auto">
        <table class="w-full text-sm">
            ...
            <tbody>
                @forelse($items as $i)
                    <tr class="border-t">
                        <td class="p-4 font-bold">{{ $i->incident_no ?? $i->ticket_no ?? $i->id }}</td>
